...
class Account extends Main_Controller {
	...
	public function index() {
        ...
        $data['customer_info'] = array(
			...
			'security_answer' 	=> '******',
			'print_invoice' 	=> site_url('account/print_invoice/'.$this->customer->getAddressId())
		);
		...
		$results = $this->Customers_model->getPaymentSubscriptionHistory($this->customer->getId());
		foreach ($results as $result) {
			$data['subscriptions_history'][] = array(
				...
				'print' 					=> site_url('account/invoice/')
			);	
