...
class Menus_model extends TI_Model {
	...
	public function getList($filter = array()) {
		...
		if ($this->db->limit($filter['limit'], $filter['page'])) {
			...
			$this->db->join('locations', 'locations.location_id = menus.menu_location');
	...
	public function getStoredProcedure($item){
		$this->db->select('*');
		$this->db->from('items_qty');
	...
	public function deleteStoredProcedure($item){
		$this->db->where_in('item_id', $item);
		$this->db->delete('items_qty');
	}

	public function getMenu($menu_id) {
		//$this->db->select('menus.menu_id, *');
		$this->db->select('menus.menu_id, menu_name, menu_description, menu_price, menu_photo, menu_category_id, stock_qty, menu_location,
	...
	public function getMenus($location_id) {
		$this->db->select('*');
		$this->db->from('menus');
		
		$this->db->where('menus.menu_location', $location_id);
	...
	public function getCategoryMenus($location_id, $category_id) {
		$this->db->from('menus');
		
		$this->db->where('menu_location', $location_id);
		$this->db->where('menu_category_id', $category_id);
	...
	public function getMenuQty($menu_id) {
		$this->db->select('*');
		$this->db->from('menus');
	...
	public function getMenuCheck($menu_id) {
		$this->db->select('*');
		$this->db->from('menus');
		$this->db->join('items_menu_association', 'items_menu_association.menu_id=menus.menu_id');
		$this->db->join('items', 'items.item_id=items_menu_association.item_id');
		$this->db->where('menus.menu_id', $menu_id);
	...
	public function updateStock($menu_id, $quantity = 0, $action = 'subtract') {
		...
		if (is_numeric($menu_id)) {
			...
			if ($query->num_rows() > 0) {
				...
				if ($row['subtract_stock'] === '1' AND ! empty($quantity) AND $row['stock_qty'] != '0' ) {
				// if ($row['subtract_stock'] === '1' AND ! empty($quantity) ) {
					...
					if ($update) {
						...
						// $items = $this->getOrderMenus($menu_id);
						// foreach ($items as $item) {
						// 	$this->Items_model->updateStock($item['item_id'], $item['required_quantity']);
						// }
						$this->load->model('ItemsMenuAssoc_model');
						$this->ItemsMenuAssoc_model->getItemOrderMenuAssoc($menu_id);
					}
				}
			}
		}
	...
	public function updateMenuQty($menu_id) {
		$this->db->select('menus.stock_qty');
		$this->db->from('menus');
		$this->db->where('menus.menu_id', $menu_id);
	...
	public function getMenuLocationName($menu_name, $location_id) {
	...
	public function saveMenu($menu_id, $save = array()) {
		...
		if (isset($save['menu_name'])) {
			$this->db->set('menu_name', ucfirst($save['menu_name']));
		}
		...
		if (isset($save['menu_location'])) {
			$this->db->set('menu_location', $save['menu_location']);
		}
	...
	public function saveAssociationMenu($menu_id, $save = array()) {
		if (empty($save) AND ! is_array($save)) return FALSE;

		if (is_numeric($menu_id)) {

			if ( ! empty($save['item_assoc'])) {
				$this->load->model('ItemsMenuAssoc_model');
				$this->ItemsMenuAssoc_model->AddItemAssoc($menu_id, $save['item_assoc']);
			}
			return $menu_id;
		}
	}
