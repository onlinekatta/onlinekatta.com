...
class Orders_model extends TI_Model {
    ...
    public function getList($filter = array()) {
        ...
        if ($this->db->limit($filter['limit'], $filter['page'])) {
            $this->db->select('*, orders.status_id, status_name, table_name, status_color, orders.date_added, orders.date_modified');
            $this->db->from('orders');
            $this->db->join('statuses', 'statuses.status_id = orders.status_id', 'left');
            $this->db->join('locations', 'locations.location_id = orders.location_id', 'left');
            $this->db->join('tables', 'tables.table_id = orders.table_id', 'left');
    ...
    public function editOrderEmail($email, $order_id){
    ...
    public function editOrderPhone($phone, $order_id){
    ...
    public function updateOrderMenu($update = array()){
        ...
        $query = $this->db->update('order_menus');
        if($query){
            $counter = $this->checkCompleted($order_id);
            if($counter == 0){
                // $datatmp = array(
                //     'order_status' => 15
                // );
                // $this->updateOrder($order_id, $datatmp);
            }
        }
    ...
    public function updateOrdersMenu($order_id, $update = array()){

        if (isset($update['quantity'])) {
            $this->db->set('quantity', $update['quantity']);
        }

        if (isset($update['total'])) {
            $this->db->set('subtotal', $update['total']);
        }
        ...
        if($query){
            $totalAmount = 0;
            $items = 0;
            $menus = $this->getOrderMenus($order_id);
            foreach ($menus as $menu) {
                $totalAmount = $totalAmount + $menu['subtotal'];
                $items = $items + $menu['quantity'];
            }

            $this->db->set('total_items', $items);
            $this->db->set('order_total', $totalAmount);
            $this->db->where('order_id', $order_id);
            $query = $this->db->update('orders');

            $totals = $this->getOrderTotals($order_id);
            foreach ($totals as $total) {
                $this->db->set('value', $totalAmount);
                $this->db->where('order_id', $order_id);
                $this->db->update('order_totals');
            }
        }
    ...
    public function checkCompleted($order_id){
    ...
    public function updateOrder($order_id = NULL, $update = array()) {
        ...
        $exist = $this->isOrderCompleted($order_id);
        if(empty($exist)):
            ...
            if (is_numeric($order_id)) {
                ...
                if ($query === TRUE) {
                    ...
                    //Substract Item Inventory Stock
                    if ( $update['order_status'] == 15 ) {
                        $this->subtractItemStock($order_id);
                        $this->subtractStock($order_id);

                        $this->checkTreshold($order_id);
    ...
    public function checkTreshold($order_id){

        $loc_data = array();

        $this->load->model('Mail_templates_model');
        $this->load->model('Dashboard_model');
        $mail_template = $this->Mail_templates_model->getTemplateData($this->config->item('mail_template_id'), 'treshold_alert');

        $location = $this->getItemInventory($order_id);
        $loc_data['location_email'] = $location['location_email'];
        $loc_data['location_manager'] = $location['location_manager'];
        $loc_data['location_name'] = $location['location_name'];
        $loc_data['location_address_1'] = $location['location_address_1'];
        $loc_data['location_city'] = $location['location_city'];
        $loc_data['location_state'] = $location['location_state'];
        $loc_data['location_postcode'] = $location['location_postcode'];

        $counter = 0;
        $inventories = $this->getItemsAssociation($location['location_id']);
        foreach ($inventories as $inventory) {
            if($inventory['item_name']){
                $counter = $counter + 1;
            }
        }
        $loc_data['counter'] = $counter;
        $loc_data['item_view_url'] = root_url('admin/dashboard');

        $this->sendSMTPMail($loc_data, $mail_template);
           
    }

    public function sendSMTPMail($loc_data=array(), $mail_template = array()) {
        
        $this->load->library('email');
        $config['protocol']    = strtolower($this->config->item('protocol'));
        $config['smtp_host']    = strtolower($this->config->item('smtp_host'));
        $config['smtp_port']    = strtolower($this->config->item('smtp_port'));
        $config['smtp_timeout'] = '7';
        $config['smtp_user']    = strtolower($this->config->item('smtp_user'));
        $config['smtp_pass']    = strtolower($this->config->item('smtp_pass'));
        ...
        $this->email->to($loc_data['location_email']); 
        $this->email->subject($mail_template['subject']);
        
        $this->email->message($mail_template['body'], $loc_data);
    ...
    public function getItemInventory($order_id)
    {
        $this->db->select('locations.location_id, location_email, location_name, location_postcode, location_state, location_city, location_address_1, order_id');
        $this->db->from('orders');
        $this->db->join('locations', 'locations.location_id=orders.location_id');
        $this->db->where('order_id',$order_id);
    ...
    public function getItemsAssociation($location_id)
    {
        $this->db->select('*');
        $this->db->from('items');
        $this->db->join('locations', 'locations.location_id=items.item_location');
        $this->db->where('items.stock_qty <=', 'items.minimum_qty');
    ...
    public function subtractItemStock($order_id) {
        //$this->load->model('Menus_model');
        
        $this->load->model('ItemsMenuAssoc_model');
        $order_menus = $this->getOrderMenus($order_id);
        foreach ($order_menus as $order_menu) {
            $this->ItemsMenuAssoc_model->getItemOrderMenuAssoc($order_menu['menu_id'], $order_menu['quantity']);
            // $this->ItemsMenuAssoc_model->getItemOrderMenuAssoc($order_menu['menu_id']);
        }
    }
    ...
    public function addOrder($order_info = array(), $cart_contents = array()) {
        ...
        $customer_id = "0";
        $emailExists = $this->Customers_model->getCustomerByEmail($order_info['email']);
        if(!empty($emailExists)){
            $customer_id = $emailExists['customer_id'];
        }
        ...
        if (isset($order_info['location_id'])) {
         ...
         if (isset($order_info['table_id'])) {
            $this->db->set('table_id', $order_info['table_id']);
        }
        ...
        if ( ! empty($order_info)) {
            ...
            if ($query AND $order_id) {
                ...
                if(empty($emailExists)){
                    ...
                    $this->Addresses_model->updateDefault($customer_id, $order_info['address_id']);
                    $this->Addresses_model->updateCustomerDefault($customer_id, $order_info['address_id']);
                    // $this->Orders_model->updateOrderAddress($customer_id, $order_id);
                }
    ...
    public function updateOrderAddress($customer_id, $order_id){
        $this->db->set('customer_id', $customer_id);
        $this->db->where('order_id', $order_id);
        $this->db->update('orders');
    }

    public function addItemOrder($order_info = array(), $cart_contents = array()) {
        if (empty($order_info) OR empty($cart_contents)) return FALSE;
        ...
        if ( ! empty($order_info)) {
            ...
            if ($order_id) {
                ...
                $this->addNewOrderMenus($order_id, $cart_contents);
    ...
    public function addNewOrderMenus($order_id, $cart_contents = array()) {
        if (is_array($cart_contents) AND ! empty($cart_contents) AND $order_id) {
            ...
            foreach ($cart_contents as $key => $item) {
                if (is_array($item) AND isset($item['rowid']) AND $key === $item['rowid']) {
                    ...
                    $itemExists = $this->getItemOrders($order_id, $item['id']);
                    if(!empty($itemExists)){
    ...
    public function getItemOrders($order_id, $menu_id) {
        if ( ! empty($order_id)) {
            $this->db->from('order_menus');
    ...
    public function deleteOrderMenu($menu_id, $order_id) {
...
/* Location: ./system/smartresto/models/orders_model.php */