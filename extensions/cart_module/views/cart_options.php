...
$(document).ready(function() {
	...
	$('.quantity-control .btn').on('click', function() {
		...
		if ($button.attr('data-dir') == 'up') {
            ...
            $.ajax({
                url:js_site_url('cart_module/cart_module/getCountItems'),
                method: 'POST',
                ...
                success:function(json){
                    ...
                    if(parseFloat(mqty) == 0){
                        alert('No order can be made on that Menu. Not available for now.');
                    }

                    if(parseInt(oldValue) < parseInt(mqty)){
                        var newVal = parseFloat(oldValue) + 1;
                        $button.parent().parent().find('#quantity').val(newVal);
                    }else{
                        alert('Can not go beyond that value: '+mqty);
                    }
                }
            });
