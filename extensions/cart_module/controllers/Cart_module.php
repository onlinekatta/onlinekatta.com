...
class Cart_module extends Main_Controller {
	...
	public function __construct() {
        ...
        $this->load->model('Items_model'); 														// load the Image tool model
	...
	public function add() {		// add() method to add item to cart
		...
		} else if ( $this->config->item('location_order') === '1' AND ! $this->location->hasSearchQuery()) {
		// if local restaurant is not selected
	...
	public function getCountItems(){
		...
		$menu = $this->Cart_model->getMenu($menu_id);

		$location = $this->Cart_model->getLocation($menu['menu_location']);
		// // print_r($massocs);
		// $datatmp = array();
		// $x = 0;
		// if(!empty($massoc)){
		...
		// 	endforeach;
		// 	$result = min($datatmp);
		if(!$location['out_range']){
			$json['qty'] = $menu['stock_qty'];
		}else{
