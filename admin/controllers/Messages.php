    ...
    private function _saveMessage() {
        if ($this->validateForm() === TRUE) {
            ...
            if ($message_id = $this->Messages_model->saveMessage($this->input->get('id'), $this->input->post())) {
                ...
                if($this->input->post("activate_social_media")){
                    // send to social media
                    $results['responsible'] = $this->user->getStaffId();
                    $social_media = $this->Social_media_model->getAll($results['responsible']);
                    foreach ($social_media as $socialmedia) {
                        if ($socialmedia->label_min === 'twitter') {
                            ...
                            $feed = $this->input->post("body");
                            $feed = str_replace("<p>","", $feed);
                            $feed = str_replace("</p>","", $feed);
                            $feed = str_replace("<br>"," ", $feed);
                             
                            $params = array(
                                'status' => $feed,
                                'media[]' => $_FILES["img"]["tmp_name"],
