...
class Orders extends Admin_Controller {
	...
	public function edit() {
		...
		$data['order_item_add'] 	= site_url('orders/order_add_item?id=' . $order_info['order_id']);
	...
	public function order_add_item(){
		...
		if ($order_info) {
			$order_id = $order_info['order_id'];
			$data['_action']	= site_url('orders/order_add_item?id='. $order_id."#menus");
		} else {
		    $order_id = 0;
			redirect('orders');
		}
        ...
        $this->template->setHeading(sprintf($this->lang->line('text_edit_heading'), $title));
         ...
         if($this->input->post()){
        	...
        	$quantityArray = $this->input->post('menuitemsquantity');
  			$cartContents = array();
  			$cartContents['total_items'] = 0;

  			if(!empty($this->input->post('menuitems')))
  			{
	        	foreach ($this->input->post('menuitems') as $key => $value) {
	        		$menuDetails = $this->Menus_model->getMenu($value);
	        		$menuDetails['menu_quantity'] = $quantityArray[$key];
	        		$menuDetails['menu_total'] = $quantityArray[$key] * $menuDetails['menu_price'];
	        		$data['menuTotalAmount'] = $data['menuTotalAmount'] + $menuDetails['menu_total'];
	        		$data['menuitems'][] = $menuDetails;

	        		$cartDetails = array();
	        		$cartDetails['id'] = $menuDetails['menu_id'];
	        		$cartDetails['name'] =  $menuDetails['menu_name'];
	        		$cartDetails['qty'] = $menuDetails['menu_quantity'];
	        		$cartDetails['price'] = $menuDetails['menu_price'];
	        		$cartDetails['subtotal'] = $menuDetails['menu_total'];
	        		$cartDetails['rowid'] = $key;
	        		$cartContents[] = $cartDetails;
	        		$cartContents['total_items'] = $cartContents['total_items'] + $menuDetails['menu_quantity'];
	        	} 
	        }

        	if(!empty($data['order_id']))
        	{
	        	$cartContents['cart_total'] = $data['menuTotalAmount'];
	        	$cartContents['order_total'] = $data['menuTotalAmount'];
	        	$cartContents['totals']['delivery'] = '';

	        	$order_id = $this->Orders_model->addItemOrder($data, $cartContents);
		...
		$this->template->render('order_item', $data);
	}
	...
	public function invoice() {
		...
		$data['width'] = $this->uri->rsegment('4');
		...
		$invoice_info = $this->Orders_model->getInvoice($this->uri->rsegment('5'));
	...
	public function add() {	
		...
		// if ($this->input->post() AND $this->_saveOrder()) {
		// 	redirect('orders');
		// }
		...
		$data['_action']	= site_url('orders/add');
		$data['menuitems'] = array();
		$data['countries'] = array();
		$data['menuTotalAmount'] = '0.0000';
		$data['country_id'] = 99;
        ...
        if($this->input->post()){
        	...
        	if($this->validateOrderAddForm() === TRUE)
        	{
	        	$cartContents['cart_total'] = $data['menuTotalAmount'];
	        	$cartContents['order_total'] = $data['menuTotalAmount'];
	        	$cartContents['totals']['delivery'] = '';

	        	$data['address_id'] = $this->Addresses_model->saveAddress(false,false,$data);

	        	$order_id = $this->Orders_model->addOrder($data, $cartContents);
				$this->Orders_model->completeOrder($order_id, $data);
		...
		$title = $this->lang->line('text_new');
        $this->template->setTitle(sprintf($this->lang->line('text_edit_heading'), $title));
        $this->template->setHeading(sprintf($this->lang->line('text_edit_heading'), $title));

		// $this->template->setButton($this->lang->line('button_save'), array('class' => 'btn btn-primary', 'onclick' => '$(\'#add-form\').submit();'));
		// $this->template->setButton($this->lang->line('button_icon_back'), array('class' => 'btn btn-default', 'href' => site_url('orders')));

		$this->template->render('order_add', $data);
	}

	public function menuautocomplete() {
		$json = array();
	...
	public function getStockQuantity() {
		...
		if ($this->input->post('id')) {
			...
			$results = $this->Menus_model->getMenuQty($menu_id);
			if ($results) {
				if($results['menu_status']):
					$json['qty'] = $results['stock_qty'];
				else:
					$json['qty'] = 0;
				endif;
			}
			// $results = $this->Menus_model->getMenuCheck($menu_id);
			// if ($results) {
			// 	foreach ($results as $result) {
			// 		if(){
	...
	public function getLocationName() {
		...
		if ($this->input->post('id')) {
			...
			$results = $this->Locations_model->getLocationName($location_id);
			if ($results) {
	...
	public function edit_email(){
		...
		$deleted_rows = $this->Orders_model->editOrderEmail($email, $order_id);
	}

	public function edit_phone(){
		$phone = $this->input->post('phone');
		$order_id = $this->input->post('order_id');
		$deleted_rows = $this->Orders_model->editOrderPhone($phone, $order_id);
	}

	public function delete_menu(){
		$menu_id = $this->input->post('menu_id');
		$order_id = $this->input->post('order_id');
		$deleted_rows = $this->Orders_model->deleteOrderMenu($menu_id, $order_id);
	}
	...
	public function edit_menu(){
		...
		$deleted_rows = $this->Orders_model->updateOrdersMenu($order_id, $datatmp);
	}

	private function validateOrderAddForm() {
		$this->form_validation->set_rules('first_name', 'lang:label_first_name', 'xss_clean|trim|required');
		$this->form_validation->set_rules('last_name', 'lang:label_last_name', 'xss_clean|trim');
		$this->form_validation->set_rules('email', 'lang:label_email', 'xss_clean|trim|required');
		$this->form_validation->set_rules('telephone', 'lang:label_telephone', 'xss_clean|trim|integer');
		$this->form_validation->set_rules('address_1', 'lang:label_address_1', 'xss_clean|trim');
		$this->form_validation->set_rules('city', 'lang:label_city', 'xss_clean|trim');
		$this->form_validation->set_rules('state', 'lang:label_state', 'xss_clean|trim');
		$this->form_validation->set_rules('order_type', 'lang:label_order_type', 'xss_clean|trim');
		$this->form_validation->set_rules('postcode', 'lang:label_postcode', 'xss_clean|trim');
		$this->form_validation->set_rules('country_id', 'lang:label_country', 'xss_clean|trim|integer');
