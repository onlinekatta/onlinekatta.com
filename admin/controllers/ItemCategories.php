...
class ItemCategories extends Admin_Controller {
	...
	public function __construct() {
        ...
        $this->load->model('ItemCategories_model'); // load the menus model
	...
	public function index() {
		...
		if ($this->input->get('sort_by')) {
			...
			$filter['sort_by'] = $data['sort_by'] = 'item_category_id';
		}
		...
		if ($this->input->post('delete') AND $this->_deleteCategory() === TRUE) {
			redirect('itemCategories');
		}

		$order_by = (isset($filter['order_by']) AND $filter['order_by'] == 'ASC') ? 'DESC' : 'ASC';
		$data['sort_name'] 			= site_url('itemCategories'.$url.'sort_by=name&order_by='.$order_by);
		$data['sort_priority'] 		= site_url('itemCategories'.$url.'sort_by=priority&order_by='.$order_by);
		$data['sort_id'] 			= site_url('itemCategories'.$url.'sort_by=item_category_id&order_by='.$order_by);

		$results = $this->ItemCategories_model->getList($filter);
		$data['itemcategories'] = array();
		foreach ($results as $result) {
			...
			$data['itemcategories'][] = array(
				...
				'edit' 					=> site_url('itemCategories/edit?id=' . $result['item_category_id'])
			);
		}
		...
		$config['base_url'] 		= site_url('itemCategories'.$url);
		$config['total_rows'] 		= $this->ItemCategories_model->getCount($filter);
		...
		$this->template->render('itemcategories', $data);
	}

	public function edit() {
		$category_info = $this->ItemCategories_model->getCategory((int) $this->input->get('id'));

		if ($category_info) {
			$category_id = $category_info['item_category_id'];
			$data['_action']	= site_url('itemCategories/edit?id='. $category_id);
		} else {
		    $category_id = 0;
			$data['_action']	= site_url('itemCategories/edit');
		}
		...
		if ($this->input->post() AND $category_id = $this->_saveCategory()) {
			...
			redirect('itemCategories/edit?id='. $category_id);
		}

        $data['item_category_id'] 		= $category_info['item_category_id'];
		...
		$data['permalink'] = $this->permalink->getPermalink('item_category_id='.$category_info['item_category_id']);
		...
		$this->template->render('itemcategories_edit', $data);
	}
	...
	private function _saveCategory() {
    	if ($this->validateForm() === TRUE) {
			...
			if ($category_id = $this->ItemCategories_model->saveCategory($this->input->get('id'), $this->input->post())) {
    ...
    private function _deleteCategory() {
        if ($this->input->post('delete')) {
            $deleted_rows = $this->ItemCategories_model->deleteCategory($this->input->post('delete'));
