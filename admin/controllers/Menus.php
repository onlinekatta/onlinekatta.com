...
class Menus extends Admin_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Locations_model'); // load the categories model
	...
	public function index() {
		...
		$data['sort_location'] = site_url('menus' . $url . 'sort_by=menu_location&order_by=' . $order_by);
	...
	public function edit() {
		...
		$data['menu_location'] = $menu_info['menu_location'];
	...
	public function checkMenu() {
		...
		if ($this->input->post('menu_name')) {
			$menu_name = ucfirst($this->input->post('menu_name'));
			$location_id = $this->input->post('location_id');

			$results = $this->Menus_model->getMenuLocationName($menu_name, $location_id);
			if ($results) {
				$json['name'] = $results['menu_name'];
			}
	...
	public function mautocomplete() {
		$json = array();

		$menu_id = $this->input->post('menu_id');
		$location_id = $this->input->post('location_id');

		$locations = $this->Locations_model->getLocation($location_id);

		$results = $this->Menus_model->getMAutoComplete($menu_id);

		// $results = $this->getCountItems($menu_id);
		if(!$locations['out_range']){
			$json['qty'] = $results['stock_qty'];
		}else{
			$json['qty'] = 100000000000;
		}
		// $json['qty'] = $results;
	...
	public function getMenus(){
    	...
    	$location_id = $this->input->post('location_id');
		$exist = $this->Menus_model->getMenus($location_id);
		if (!empty($exist)) {
            $rec=0;
            $return_data = array();
            foreach($exist as $value) {
                $return_data[$rec]['menu_id'] = $value['menu_id'];
                $return_data[$rec]['menu_name'] = $value['menu_name'];
                $return_data[$rec]['menu_price'] = $value['menu_price'];
                $return_data[$rec]['menu_photo'] = $value['menu_photo'];
    ...
    public function CategoryMenus(){
		...
		$exist = $this->Menus_model->getCategoryMenus($location_id, $category);
		if (!empty($exist)) {
            $rec=0;
            $return_data = array();
            foreach($exist as $value) {
	...
	private function validateForm() {
		...
		$this->form_validation->set_rules('menu_location', 'lang:label_location', 'xss_clean|trim|required|integer');
