...
class Customer_groups extends Admin_Controller {
    ...
    public function index() {
        ...
        // $location = $this->Customer_groups_model->getCustomerGroup(12);
        // print_r($location);
        // echo $location['location_id'];
        //       $exists = $this->Locations_model->getLocationName($location['location_id']);
        //       print_r($exists);
        //       echo $exists['subscription'];
    ...
    private function _deleteCustomerGroup() {
        if ($this->input->post('delete')) {
            ...
            $location = $this->Customer_groups_model->getCustomerGroup($this->input->post('delete'));
            $exists = $this->Locations_model->getLocationName($location['location_id']);

            if ($exists['subscription'] AND $location['group_name'] == 'Subscription Specific Users') {
                $this->alert->set('warning', sprintf($this->lang->line('alert_error_no_deletion'), $this->lang->line('text_deleted')));
            } else {
